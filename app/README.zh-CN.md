...
[![最新版本](https://img.shields.io/github/v/release/dakebubi-7/DailyTodo?display_name=tag&style=flat-square)](https://github.com/dakebubi-7/DailyTodo/releases)
[![许可证](https://img.shields.io/github/license/dakebubi-7/DailyTodo?style=flat-square)](LICENSE)
...
安装包和解压版程序会生成在 `release/`。构建产物被 `.gitignore` 排除，避免个人配置、日志和二进制文件进入源码仓库。

## 项目结构

```text
electron/   Electron 主进程、IPC、持久化、AI 和集成服务
src/        React 渲染进程、Hooks、组件、设置和样式
shared/     跨进程类型以及 AI/复盘契约
tests/      单元测试、DOM 测试和回归测试
scripts/    验证脚本与原生模块构建辅助脚本
docs/       产品规格、实现计划和发布说明
```
...
- [发布说明](RELEASE_NOTES_2026-08-20.md)
- [版本记录](CHANGELOG.md)
