...
const taskViewSelectorPath = join(root, 'src/components/taskList/TaskViewSelector.tsx');
assert(existsSync(taskViewSelectorPath), 'Task view selector component should exist.');
const taskViewSelector = readFileSync(taskViewSelectorPath, 'utf8');
const reviewViewPath = join(root, 'src/components/ReviewView.tsx');
assert(existsSync(reviewViewPath), 'ReviewView component should exist.');
const reviewView = readFileSync(reviewViewPath, 'utf8');
...
assert(taskListToolbar.includes("import { memo, useState } from 'react';"), 'TaskListToolbar should import memo and local filter-expansion state.');
...
assert(taskListToolbar.includes('task-tool-icon') && taskListToolbar.includes('task-filter-launcher'), 'TaskListToolbar should render icon-only search and filter launchers.');
assert(taskList.includes('activeTab={activeTab}') && taskList.includes('onTabChange={onTabChange}'), 'TaskList should pass the active task view and setter into its toolbar.');
assert(taskListToolbar.includes("from './TaskViewSelector'") && taskListToolbar.includes('<TaskViewSelector text={text} activeTab={activeTab} onTabChange={onTabChange} />'), 'TaskListToolbar should render the shared compact task-view selector.');
assert(taskViewSelector.includes('task-view-launcher') && taskViewSelector.includes('task-view-menu-popover'), 'TaskViewSelector should own the compact task-view selector and its popover.');
assert(taskViewSelector.includes('role="menuitemradio"') && taskViewSelector.includes('onTabChange(option.value)'), 'TaskViewSelector should route exclusive view selection through the existing tab state callback.');
assert(reviewView.includes("from './taskList/TaskViewSelector'") && reviewView.includes('<TaskViewSelector text={text} activeTab={activeTab} onTabChange={onTabChange} />'), 'ReviewView should retain the shared task-view selector so users can leave review mode.');
assert(appShellMainContentComposition.includes('const reviewViewProps = {') && appShellMainContentComposition.includes('text: shellText.app,') && appShellMainContentComposition.includes('activeTab,') && appShellMainContentComposition.includes('onTabChange: setActiveTab,'), 'ReviewView props should receive localized shared task-view state.');
assert(/\.task-toolbar\s*\{[^}]*position:\s*relative;[^}]*z-index:\s*2;/.test(globals), 'Task toolbar should stack above task content so the task-view menu remains clickable.');
assert(taskListToolbar.includes('aria-label={text.filterTasks}'), 'Filter launcher needs a localized accessible name.');
assert(/isFilterOpen && \([\s\S]*task-filter-select/.test(taskListToolbar), 'Existing filter controls should remain available in an expandable region.');
assert(taskListToolbar.includes('task-daily-actions'), 'TaskListToolbar should own the right-aligned daily editor actions.');
assert(taskListToolbar.includes('onToggleDailyWorkPanel') && taskListToolbar.includes('onToggleInspirationPanel'), 'TaskListToolbar should receive both daily editor actions.');
