...
const popupSourcePane = existsSync(popupSourcePanePath) ? readFileSync(popupSourcePanePath, 'utf8') : '';
...
assert(existsSync(popupSourcePanePath), 'Task-source pane UI should live in a focused task-menu popup module.');
...
assert(popupEntry.includes("from './taskMenuPopup/TaskMenuPopupSourcePane'"), 'TaskMenuPopup should import the extracted task-source pane.');
...
assert(popupSourcePane.includes('export function TaskMenuPopupSourcePane'), 'Task-source pane module should export TaskMenuPopupSourcePane.');
assert(popupSourcePane.includes("task.source || 'personal'"), 'Task-source pane should treat missing task sources as personal.');
assert(popupSourcePane.includes('onDispatch(task.id, { source })'), 'Task-source pane should dispatch a source-only task update.');
assert(popupSourcePane.includes("disabled={currentSource === 'personal'}"), 'Task-source pane should disable the current personal source.');
assert(popupSourcePane.includes("disabled={currentSource === 'external'}"), 'Task-source pane should disable the current external source.');
assert(popupPanes.includes("| 'source'"), 'Task-menu pane state should include a task-source pane.');
assert(popupPanes.includes("onPick('source')"), 'Task-menu should open the task-source pane from its type control.');
assert(popupEntry.includes("pane === 'source'"), 'TaskMenuPopup should render the task-source pane.');
assert(popupEntry.includes('<TaskMenuPopupSourcePane task={task} onBack={() => setPane(\'menu\')} onDispatch={dispatch} onClose={close} />'), 'TaskMenuPopup should wire source changes through the existing dispatch and close handlers.');
