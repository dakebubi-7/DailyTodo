...
日报模板生成有 3 个触发入口:
1. **手动触发**:任务列表/产品主页"保存并同步"按钮(沿用现有行为,本轮不动入口位置,只接入新模板系统)
2. **每日定时**:沿用现有每日定时器(本轮不动)
3. **重新生成今日日报**:在设置区"周/月报自动生成"末尾加 1 个"立即重新生成今日日报"按钮(与 4 个周月报按钮并列,共 5 个立即生成按钮),点击后用新模板重新生成今天的日报文件(覆盖原文件,但保留 3 块原始内容)
