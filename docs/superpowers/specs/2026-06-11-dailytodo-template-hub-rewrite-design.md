...
3. **AI ✓ toggle** → 关闭后,该块渲染类型下拉变灰,下方提示"该块已切换为手动编辑"。**已有 AI 内容保留(冻结)**,不再被 LLM 覆盖,用户可手动编辑或重新开启 AI ✓ 后下一次生成时由 LLM 替换。
...
7. **📎 上传 .md/.txt 让 AI 识别** → 弹二级模态:
   ...
   **冲突处理:** AI 识别结果中若出现与固定区块(今日工作/灵感随笔/每日任务)**重名**的自定义块,弹确认 "识别到与固定区块重名的区块,已自动忽略 N 个,继续?",用户确认后忽略重名项。
...
两个 marker 之间留一空行,作为后续 AI 内容的容器。`runner.ts` 的 `upsertBlock` 读取 START/END 之间的内容作为已有内容,写入时替换 START/END 之间的全部内容。
...
```
渲染"工作总结"块时(根据"work" / "总结" / "summary" 关键词匹配):
  切片 B = [每天的"今日工作"详情]
...
**块名匹配规则(切片 B vs 切片 A):** 自定义块的 `name` 字段若包含"工作"/"总结"/"summary"/"work" 关键词 → 走切片 B(单独调一次 LLM,接收工作详情);否则 → 走切片 A(与其他块一起调一次 LLM,接收非工作切片)。**用户可手动重命名块的 `name` 来切换切片归属。**
...
仅对 `externalWeekly*` / `externalMonthly*` 报告生效。**脱敏在 AI 内容生成并写入 marker 后、最终文件落盘前执行**(一次性原子替换)。

**轻量脱敏规则(`lightAnonymize`):**
- 替换人名为 `[人员]`(基于内置常见称呼正则:`张三`/`李四`/`王五` + 2-3 字中文姓名,以及 `@xxx` 提及)
- 替换项目代号为 `[项目A]` `[项目B]`(基于配置的项目白名单,本次内置 2 个示例正则:`项目\s*\d+` / `客户\s*[一-龥A-Z]{2,8}`)
- 替换电话号码为 `[联系方式]`(`1[3-9]\d{9}` 11 位手机号)
- 替换邮箱为 `[联系方式]`(`\w+@\w+\.\w+`)
...
- **所有自定义块 `aiGenerate=false`** → AI 填充层**完全跳过**,只渲染模板层(手动编辑或空)
- **0 个自定义块** → 模板只渲染固定块,正常写入

### 3.7 时区与并发

- 所有触发时间使用 **OS 本地时区**(沿用现有行为,本轮不变)
- 用户在生成进行中修改设置 → 已开始的生成使用**生成开始时**的快照配置,新配置从下一次生成生效
- 同一时间多个触发重叠 → 串行执行,不在并发上做特殊处理(由 `runner.ts` 的现有队列保证)
...
- 模板的导入/导出(用户间分享)
- 模板版本管理
- 历史的"补生成 N 天"能力(`backfillDays`) → 简化为"重新生成指定周/月"4 个按钮,无 N 天回溯

---

## 十一、补充说明

### 11.1 日报触发:"主界面同步按钮" vs 设置区"立即重新生成今日日报"

**主界面"同步"按钮**(智能,日常使用):
- 行为:写入/更新今日 Obsidian 日报
- 写入策略:
  - **3 块原始数据**(今日工作 / 灵感随笔 / 每日任务):总是写入(取自产品编辑框和任务列表)
  - **N 个 AI 自定义块**:扫 marker 状态
    - 若 marker 内为空 → 调 LLM 生成
    - 若 marker 内已有内容 → 保留不动(当天首次同步后,第二次同步不会覆盖已生成的 AI 内容)
- 适用:用户写完当天内容后,想看一次 AI 复盘

**设置区"立即重新生成今日日报"**(强制,调试用):
- 行为:无条件重新生成今日日报的所有 AI 块
- 写入策略:
  - **3 块原始数据**:保留不动(取自产品编辑框,**不**覆盖用户已写入 Obsidian 的)
  - **N 个 AI 自定义块**:`aiGenerate=true` 块**强制调 LLM 重写**,覆盖已有内容
- 适用:用户改了模板/提示词,想立刻看新模板生成的效果
- 二次确认:弹"将覆盖今日 AI 块已有内容,确定?"(避免误点丢数据)

**日报其他触发入口(本轮不动):**
- 每日定时器(沿用现有)
- AI 复盘面板的"立即生成本周/本月/对外周/对外月"按钮(**不**触发日报)
...
### 11.2 路径变量与 `obsidianPath` 的关系

`obsidianPath`(Vault 根目录)与 5 条路径字段的关系:

- 5 条路径字段存储**相对 Vault 根的相对路径**(如 `logs/daily/{{date}}.md`)
- 生成文件时,实际写入路径 = `obsidianPath` + 路径字段(变量替换后)
- 若 `obsidianPath` 为空,跳过同步(状态栏提示"未选择 Vault")
- 变量替换(`{{date}}` / `{{year}}` / `{{month}}` / `{{week}}`)在拼接前完成,变量未识别时保留原样并提示

### 11.3 老 `dailyMarkdownTemplate` 自定义顺序的迁移

若老模板使用 `{{work}} {{inspire}} {{tasks}}` 三 token 且**指定了非常规顺序**(如 `{{tasks}} {{inspire}} {{work}}`),迁移时:
- 按 token 在原文中的出现顺序解析 → 还原 `fixedBlocks` 数组顺序
- 若只出现部分 token(如只有 `{{work}}` 和 `{{inspire}}`),缺失的 token 走默认位置(末尾补齐),不报错
- 若 token 全部缺失或老模板为空,固定块走默认顺序(今日工作 / 灵感随笔 / 每日任务)
